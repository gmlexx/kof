...
dev-adopted-deploy: dev kind envsubst ## Create adopted cluster deployment
	...
	NAMESPACE=$(KCM_NAMESPACE) KUBECONFIG_DATA=$$($(KIND) get kubeconfig --internal -n $(KIND_CLUSTER_NAME) | base64 -w 0) KIND_CLUSTER_NAME=$(KIND_CLUSTER_NAME) $(ENVSUBST) -no-unset -i demo/creds/adopted-credentials.yaml | $(KUBECTL) apply -f -
...
dev-regional-deploy-adopted: dev ## Deploy regional adopted cluster using k0rdent
	...
	@for attempt in $$(seq 1 20); do \
		...
		for name in 'cert-manager' 'ingress-nginx' 'kof-operators' 'kof-storage' 'kof-collectors' ; do \
			helm_status=$$($(HELM) list --kube-context kind-regional-adopted --deployed --pending --superseded -o yaml -A | $(YQ) ".[] | select(.name == \"$$name\") | .status"); \
